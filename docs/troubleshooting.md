...
| `--split --yes` stops before API call / split 非交互在 API 前停止       | `sensitive_data` / `7`          | Complete untracked scan found sensitive-looking data / 完整未跟踪扫描发现疑似敏感数据                                                               | Review/stage intended files explicitly; do not bypass without checking the actual content                              |
...
| Split stopped after one or more commits / split 部分提交后停止          | reported Git failure            | Hook, crash, SIGINT, or concurrent pending edit / hook、崩溃、中断或待处理文件变化                                                                  | Resolve the cause without deleting `.git/aicommit`, then run `aicommit split --resume`                                 |
