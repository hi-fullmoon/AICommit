...
for (const strategy of ['auto', 'deep'])
  for (const split of [false, true]) {
    test(
      'large-change CLI preserves mode and reports coverage: ' +
        strategy +
        ':' +
        (split ? 'split' : 'single'),
      async (t) => {
        ...
        if (strategy === 'auto') {
          assert.ok(output.data.analysis.sampledFiles > 0);
          assert.ok(output.data.analysis.sampledFiles < 111);
          assert.ok(output.warnings.some((warning) => warning.includes('not fully analyzed')));
        }
        assert.equal(output.data.analysis.requests, requests);
        assert.equal(output.usage.totalTokens, requests * 15);
        if (strategy === 'deep') assert.ok(requests > 1);
